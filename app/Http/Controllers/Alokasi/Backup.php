...
class AlokasiController extends Controller
{
	function get_kegiatan(Request $request){
		...
		$data_kegiatan = DB::connection($conn['conn_status'])->select("
			...
			FROM (
			SELECT k.kode_kegiatan,k.nama_kegiatan,k.npsn,
			SUM(COALESCE(kd.bulan_1,0)) as bulan_1,
			SUM(COALESCE(kd.bulan_2,0)) as bulan_2,
			SUM(COALESCE(kd.bulan_3,0)) as bulan_3,
			SUM(COALESCE(kd.bulan_4,0)) as bulan_4,
			SUM(COALESCE(kd.bulan_5,0)) as bulan_5,
			SUM(COALESCE(kd.bulan_6,0)) as bulan_6,
			SUM(COALESCE(kd.bulan_7,0)) as bulan_7,
			SUM(COALESCE(kd.bulan_8,0)) as bulan_8,
			SUM(COALESCE(kd.bulan_9,0)) as bulan_9,
			SUM(COALESCE(kd.bulan_10,0)) as bulan_10,
			SUM(COALESCE(kd.bulan_11,0)) as bulan_11,
			SUM(COALESCE(kd.bulan_12,0)) as bulan_12,
			SUM(COALESCE(kd.nilai,0)) as anggaran
			FROM budget2021.kegiatan as k
			LEFT JOIN budget2021.kegiatan_detail as kd ON k.kode_kegiatan=kd.kode_kegiatan
			WHERE kd.npsn='$npsn' AND (k.kode_kegiatan like '%$kode%' AND k.kode_kegiatan NOT IN ('5.05.3.03.016','5.05.3.01.048')) AND k.npsn=kd.npsn
		...
		// CEK GTT PTT
		$get_gtt_ptt = DB::connection($conn['conn_status'])->table('budget2021.kegiatan')->whereRaw("npsn='$npsn' AND (kode_kegiatan='5.05.3.03.016' AND kode_kegiatan LIKE '%$kode%')")->first();
		if(!empty($get_gtt_ptt)){
			...
			if($request->status=='1'){
				$where = "sdm.npsn='$npsn' AND sdm.kode_kegiatan = '5.05.3.03.016' AND sdm.komponen_id IN($str_komponen) ";
			}else{
			...
			$cek_data_honor = DB::connection($conn['conn_status'])->select("
				...
				sdm.kode_kegiatan,
				'$get_gtt_ptt->nama_kegiatan' as nama_kegiatan,
				sdm.npsn,
				...
				WHERE $where AND (sdm.status_perangkaan='1' AND sdm.lock='1')
				GROUP  BY sdm.kode_kegiatan,sdm.npsn) as datanya
				");

			if(count($cek_data_honor)!=0){
				$baris = $cek_data_honor[0];
				...
				if(count($data_kegiatan)!=0){
					array_push($data_kegiatan,$cek_data_honor[0]);
				}else{
					$data_kegiatan = [$cek_data_honor[0]];
				}
			}
		}
		// END CEK GTT PTT
		...
		$cek_pelatih = DB::connection($conn['conn_status'])->table('budget2021.kegiatan')->selectRaw("*")->whereRaw("(kegiatan.kode_kegiatan = '5.05.3.01.048' AND kegiatan.kode_kegiatan like '%$kode%') AND npsn='$npsn'")->first();
		if(!empty($cek_pelatih)){
			...
			$cek_data_pelatih_disetujui = DB::connection($conn['conn_status'])->select("
				SELECT *,(bulan_1+bulan_2+bulan_3+bulan_4+bulan_5+bulan_6+bulan_7+bulan_8+bulan_9+bulan_10+bulan_11+bulan_12) as total,
				anggaran-(bulan_1+bulan_2+bulan_3+bulan_4+bulan_5+bulan_6+bulan_7+bulan_8+bulan_9+bulan_10+bulan_11+bulan_12) as sisa 
				FROM (
				SELECT 
				SUM(COALESCE(sdm.nominal,0)) as anggaran,
				sdm.kode_kegiatan,
				'$cek_pelatih->nama_kegiatan' as nama_kegiatan,
				sdm.npsn,
				SUM(COALESCE(sd.bulan_1,0)) as bulan_1,
				SUM(COALESCE(sd.bulan_2,0)) as bulan_2,
				SUM(COALESCE(sd.bulan_3,0)) as bulan_3,
			...
			if(count($cek_data_pelatih_disetujui)!=0){
				$baris = $cek_data_pelatih_disetujui[0];
				...
				if(count($data_kegiatan)!=0){
					array_push($data_kegiatan,$cek_data_pelatih_disetujui[0]);
				}else{
					$data_kegiatan = [$cek_data_pelatih_disetujui[0]];
				}
			}
		}
		// END CEK PELATIH
	...
	function simpan_bulanan(Request $request){
		...
		if($kode_kegiatan=='5.05.3.01.048' || $kode_kegiatan=='5.05.3.03.016' || $kode_kegiatan=='5.05.3.03.016'){
